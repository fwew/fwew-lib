...
func normalAdjective(allAdjectives []Word, dialect int, twoWordNoun bool) (adj string) {
...
func genitiveNoun(allNouns []Word, dialect int) (adj string) {
...
func originNoun(allNouns []Word, dialect int, twoWordNoun bool) (adj string) {
	adjWord := fastRandom(allNouns)
	adj = strings.ToLower(adjWord.Navi)
	switch adj {
	case "tsko swizaw":
...
func participleVerb(allVerbs []Word, dialect int, twoWordNoun bool) (adj string) {
	infix := "us"
...
func activeParticipleVerb(allVerbs []Word, dialect int, twoWordNoun bool) (adj string) {
	findVerb := oneWordVerb(allVerbs)
	adj = findVerb.InfixDots
	switch dialect {
	case 2: // reef
...
func passiveParticipleVerb(allTransitiveVerbs []Word, dialect int, twoWordNoun bool) (adj string) {
...
func getAdjMode(adjMode int) (amode int) {
	// Adjective
	amode = 0
	switch adjMode {
	case 0:
...
func getNMode(nounMode int) (nmode int) {
...
func getNameAluNoun(nmode int, dialect int, allNouns, allVerbs []Word) (noun string) {
	switch nmode {
	case 1:
...
func getNameAluAdjective(amode int, allAdjectives []Word, dialect int, twoWordNoun bool, allNouns, allVerbs, allTransitiveVerbs []Word) (adj string) {
	switch amode {
	// no case 1 (no adjective)
	case 2: // normal adjective
		adj = normalAdjective(allAdjectives, dialect, twoWordNoun)
	case 3: //genitive noun
		adj = genitiveNoun(allNouns, dialect)
	case 4: //origin noun
		adj = originNoun(allNouns, dialect, twoWordNoun)
	case 5: //participle verb
		adj = participleVerb(allVerbs, dialect, twoWordNoun)
	case 6: //active participle verb
		adj = activeParticipleVerb(allVerbs, dialect, twoWordNoun)
	case 7: //passive participle verb
		adj = passiveParticipleVerb(allTransitiveVerbs, dialect, twoWordNoun)
	}
	return
}
